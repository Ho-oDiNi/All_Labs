...
void BigChars(int value)
{
    ...
    switch(value)
    {
    	...
    	case 2:
    		for(int i = 0; i < 4; i++)		
    			bc_printbigchar(bcint2, 8*i+9, 13, cl_green, cl_default );
    	break;
    	//
    	case 9:
    		for(int i = 0; i < 4; i++)		
...
void displayBigChars(int value, int i, int j)
{
...
{
    mt_gotoXY (64, 2);
    printMemoryCell(accumulator);
    bc_box(63, 1, 84, 3);
    mt_gotoXY (65, 1);
    cout<<"accumulator";
}

void displayAccumulator(int &accumulator, int i, int j)
{
		sc_memoryGet(i, j, accumulator);
    mt_gotoXY (64, 2);
    printMemoryCell(accumulator);
    bc_box(63, 1, 84, 3);
    mt_gotoXY (65, 1);
...
void displayInctructionCounter(int i, int j)
{
		mt_gotoXY(66, 5);
    cout<< "0" << i << ":0"<< j;
    bc_box(63, 4, 84, 6);
...
int main()
{
    ...
    //+4BigChars
    displayBigChars(value, 0, 0);
    
    //accumulator
    int accumulator;
    displayAccumulator(accumulator, 0, 0);
    
    //inctructionCounter
    displayInctructionCounter(0, 0);
    		...
    		{	
    		  ...
    		  case KEY_right://вправо
					...
					case KEY_f6: //f6
							...
							mt_gotoXY(70, 5);
							cin>>i;
              displayInctructionCounter(i, j);
							break;
	  	
           case KEY_l:  //l
              displayAccumulator(accumulator, i, j);
              break;

            case KEY_s: //s
               sc_memorySet(i, j, accumulator);
               displayAccumulator(accumulator, i, j);
               BigChars(accumulator);
               break;

            case KEY_i: //i
               for(int i1 = 0; i1 < SIZE; i1++) 
               for (int j1 = 0; j1 < SIZE; j1++) 
               		sc_memorySet(i1,j1, 0);
							 displayMemory(value);
               break;				
    		}	
    		
    		displayInctructionCounter(i, j);
    		displayBigChars(value, i, j);
    		mt_gotoXY(1, 23);
    }

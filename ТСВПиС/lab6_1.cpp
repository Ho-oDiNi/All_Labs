...
int main() 
{
	...
	{
		cin >> temp;
  		F2[i].real(temp); F2[i].imag(0);
	}
	
	
	for(int i=0; i<size1; i++)
	{
		F1[i+size1].real(0); F1[i].imag(0);
	}
	for(int i=0; i<size1; i++)
	{
		F2[i+size2].real(0); F2[i].imag(0);
	}
	...
	for(int i=0; i<n; i++)
	{
		A[i].real(   A1[i].real() * A2[i].real() -  A1[i].imag() * A2[i].imag()   );
		A[i].real(   A1[i].real() * A2[i].real() -  A1[i].imag() * A2[i].imag()   );
	}
